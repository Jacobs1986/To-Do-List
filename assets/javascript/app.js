...
// Global variables
let listCount = 1;
let toDoArray = JSON.parse(localStorage.getItem('list'));

// get the list from local storage
console.log(toDoArray);

// generate styling for the list
toDoGenerate = toDoItem => {
     // Set $("<p>") to a variable
     let paragraph = $("<p>");
...
toDoArray.forEach(toDoGenerate);
...
$(document.body).on("click", ".checkbox", function () {
    ...
    toDoArray.splice((toDoNumber - 1), 1);
    console.log(toDoArray)
    // save to localStorage
    localStorage.setItem('list', JSON.stringify(toDoArray));
